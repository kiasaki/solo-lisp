...
use types::{SoloVal, SoloRet, _nil, list, err_string};
use types::SoloType::{Sym, List, Vector};

struct EnvType {
    data: HashMap<String,SoloVal>,
    outer: Option<Env>,
}
...
pub fn env_bind(env: &Env,
                mbinds: SoloVal,
                mexprs: SoloVal) -> Result<Env,String> {
...
pub fn env_find(env: &Env, key: &SoloVal) -> Option<Env> {
    match **key {
        Sym(ref k) => {
            let map = env.borrow();
...
pub fn env_set(env: &Env, key: SoloVal, val: SoloVal) {
    match *key {
...
pub fn env_get(env: &Env, key: &SoloVal) -> SoloRet {
    match **key {
        Sym(ref k) => {
