...
// General functions
fn equal_q(a: Vec<SoloVal>) -> SoloRet {
    if a.len() != 2 {
...
// Errors/Exceptions
fn throw(a: Vec<SoloVal>) -> SoloRet {
    if a.len() != 1 {
...
// String routines
fn pr_str(a: Vec<SoloVal>) -> SoloRet {
    Ok(string(printer::pr_list(&a, true, "", "", " ")))
}

fn str(a: Vec<SoloVal>) -> SoloRet {
    Ok(string(printer::pr_list(&a, false, "", "", "")))
}

fn prn(a: Vec<SoloVal>) -> SoloRet {
...
fn readline(a: Vec<SoloVal>) -> SoloRet {
    match *a[0] {
        Strn(ref a0) => match readline::solo_readline(&a0) {
...
fn read_string(a: Vec<SoloVal>) -> SoloRet {
    match *a[0] {
...
fn slurp(a: Vec<SoloVal>) -> SoloRet {
    match *a[0] {
        Strn(ref a0) => {
...
// Numeric functions
fn int_op<F>(f: F, a: Vec<SoloVal>) -> SoloRet
...
fn bool_op<F>(f: F, a: Vec<SoloVal>) -> SoloRet
...
pub fn add(a: Vec<SoloVal>) -> SoloRet { int_op(|i,j| { i+j }, a) }
...
pub fn gt (a: Vec<SoloVal>) -> SoloRet { bool_op(|i,j| { i>j }, a) }
...
// Hash Map functions
pub fn assoc(a: Vec<SoloVal>) -> SoloRet {
    if a.len() < 3 {
...
pub fn dissoc(a: Vec<SoloVal>) -> SoloRet {
    if a.len() < 2 {
...
pub fn get(a: Vec<SoloVal>) -> SoloRet {
    if a.len() != 2 {
...
pub fn contains_q(a: Vec<SoloVal>) -> SoloRet {
    if a.len() != 2 {
...
pub fn keys(a: Vec<SoloVal>) -> SoloRet {
    if a.len() != 1 {
...
pub fn vals(a: Vec<SoloVal>) -> SoloRet {
    if a.len() != 1 {
...
// Sequence functions
pub fn cons(a: Vec<SoloVal>) -> SoloRet {
    match *a[1] {
...
pub fn concat(a: Vec<SoloVal>) -> SoloRet {
    let mut new_v: Vec<SoloVal> = vec![];
    for lst in a.iter() {
        match **lst {
            List(ref l,_) | Vector(ref l,_) => {
                // TODO replace by when stable new_v.push_all(l);
                for i in l.iter() {
                    new_v.push(i.clone());
                }
            },
...
pub fn nth(a: Vec<SoloVal>) -> SoloRet {
    if a.len() != 2 {
...
pub fn empty_q(a: Vec<SoloVal>) -> SoloRet {
    if a.len() != 1 {
...
pub fn count(a: Vec<SoloVal>) -> SoloRet {
    if a.len() != 1 {
        return err_str("Wrong arity to count call");
    }
    match *a[0] {
        List(ref v,_) | Vector(ref v,_) => Ok(_int(v.len() as isize)),
        Nil => Ok(_int(0)),
        _ => err_str("count called on non-sequence"),
    }
}

pub fn apply(a: Vec<SoloVal>) -> SoloRet {
    if a.len() < 2 {
    ...
    match *a[a.len()-1] {
        List(ref v, _) | Vector(ref v, _) => {
            // TODO replace by when stable args.push_all(v);
            for i in v.iter() {
                args.push(i.clone());
            }
            f.apply(args)
...
pub fn map(a: Vec<SoloVal>) -> SoloRet {
    if a.len() != 2 {
        return err_str("Wrong arity to map call");
    }
    let mut results:Vec<SoloVal> = vec![];
    match *a[1] {
        List(ref v,_) | Vector(ref v,_) => {
            for mv in v.iter() {
...
pub fn conj(a: Vec<SoloVal>) -> SoloRet {
    if a.len() < 2 {
...
// Metadata functions
fn with_meta(a: Vec<SoloVal>) -> SoloRet {
    if a.len() != 2 {
    ...
    match *a[0] {
        ...
        SoloFunc(ref mfd,_) => Ok(solofuncd(mfd.clone(), meta)),
        Func(f,_) => Ok(funcm(f, meta)),
        _ => err_str("type does not support metadata"),
    }
}

fn meta(a: Vec<SoloVal>) -> SoloRet {
    if a.len() != 1 {
...
// Atom functions
fn deref(a: Vec<SoloVal>) -> SoloRet {
    if a.len() != 1 {
...
fn reset_bang(a: Vec<SoloVal>) -> SoloRet {
    if a.len() != 2 {
        return err_str("Wrong arity to map call");
    }
    match *a[0] {
...
fn swap_bang(a: Vec<SoloVal>) -> SoloRet {
    if a.len() < 2 {
