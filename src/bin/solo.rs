...
use solo::types::{SoloVal, SoloRet, SoloError, err_str};
use solo::types::{symbol, _nil, string, list, vector, hash_map, solofunc, solofuncd};
use solo::types::SoloError::{ErrString, ErrSoloVal};
use solo::types::SoloType::{Nil, False, Sym, List, Vector, Hash_Map, Func, SoloFunc};
use solo::{readline, reader, core};
use solo::env::{env_set, env_get, env_new, env_bind, env_find, env_root, Env};


// read
fn read(str: String) -> SoloRet {
    reader::read_str(str)
}

// eval
fn is_pair(x: SoloVal) -> bool {
    match *x {
...
fn quasiquote(ast: SoloVal) -> SoloVal {
...
fn is_macro_call(ast: SoloVal, env: Env) -> bool {
    let lst = match *ast {
    ...
    match *f {
        SoloFunc(ref mfd,_) => mfd.is_macro,
        _ => false,
    }
}

fn macroexpand(mut ast: SoloVal, env: Env) -> SoloRet {
    while is_macro_call(ast.clone(), env.clone()) {
        ...
        match *mf {
            SoloFunc(_,_) => ast = try!(mf.apply(args[1..].to_vec())),
            _ => break,
        }
    }
    Ok(ast)
...
fn eval_ast(ast: SoloVal, env: Env) -> SoloRet {
    match *ast {
        ...
        Hash_Map(ref hm,_) => {
            let mut new_hm: HashMap<String,SoloVal> = HashMap::new();
...
fn eval(mut ast: SoloVal, mut env: Env) -> SoloRet {
    'tco: loop {
    ...
    match a0sym {
        ...
        "fn*" => {
            let a1 = args[1].clone();
            let a2 = args[2].clone();
            return Ok(solofunc(eval, a2, env, a1, _nil()));
        },
        "eval" => {
            let a1 = (*args)[1].clone();
...
// print
fn print(exp: SoloVal) -> String {
    exp.pr_str(true)
}

fn rep(str: &str, env: Env) -> Result<String,SoloError> {
...
(def! load-file (fn* (f)
  (do (println *FOLDER*) (def! *FILENAME* (str *FOLDER* *PATH_SEPARATOR* f))
      (def! *FOLDER* (file-folder *FILENAME*))
      (println (str *FOLDER* " - " *FILENAME*))
      (eval (read-string
  ...
  nil)))
    ...
    if args.len() > 1 {
        ...
        return match rep(&lf, repl_env.clone()) {
            ...
            Err(str) => {
                println!("Error: {:?}", str);
                // TODO add exit code 1 when exit codes become stable
                return;
            }
        };
    }
